...
- fix: TIC Y-axis crash — `labels` argument requires `at` in `graphics::axis()` (v1.3.2)
...
- perf: TIC PNG rendered at 2× resolution (res=144) for crisp antialiased text; dimensions now match ion image exactly for seamless view transition (v1.3.4): window halved to ±0.5 Da, white hairline bars, 50% opacity marker line, axis-matched peak label size, y-axis gap removed
