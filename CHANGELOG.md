...
- fix: the two remaining `except: pass` blocks (S3 cleanup after a dataset/project delete) now log instead of vanishing — a failed cleanup used to leak orphaned S3 objects that nothing would ever report. CI now rejects new silently-swallowed exceptions (`ruff` S110/E722): swallowing is still allowed where it's right, but it has to be a deliberate, reviewed choice rather than an accident. This is the class of defect that hid dead ML scoring for months
- fix: **CI could report a successful deploy while the site was down for everyone.** Every check inspected the box from inside the box — the post-deploy probe curls `http://localhost:8000`, bypassing nginx, TLS and DNS — so when the HTTPS certificate expired on 2026-08-07 and every user (plus Vercel's `/api/*` proxy) got a TLS failure, the deploy still went green. A new `public-probe` CI job now requests `https://api.peakme.now/health` with certificate validation on, fails the run if the public endpoint is unhealthy, and warns 14 days before the certificate expires
- fix: the documented certbot renewal cron could never have worked (and the first replacement was itself wrong — cron has no line-continuation syntax, so a multi-line entry is a parse error; it is now a single line installed non-interactively) — it installed into the `ubuntu` user's crontab (certbot needs root for `/etc/letsencrypt`) and used `--standalone`, which cannot bind port 80 while the nginx container holds it. It only restarted nginx afterwards, never stopping it first. Both failures were silent, which is why the certificate expired. `docs/deployment.md` §9 now documents a root crontab with pre/post hooks, plus a `--dry-run` verification step
- feat: **account linking** — one person can hold several sign-ins (Google vs magic link, work vs personal address), and Supabase makes a separate account for each. Because ownership pointed at a single account, whether you could delete your own project depended on *which button you used to sign in*. Linked accounts are now treated as the same person, so permissions follow you rather than your login method. Purely additive (migration 0007, nullable column, no backfill): existing accounts are unaffected until deliberately linked, and every ownership check now runs through one function so real multi-user support is a change in one place (ADR-016, 8 tests)
...
- fix: offline annotations are now tagged with the account that made them, so on a shared device they can no longer be replayed under whoever signs in next and permanently misattributed. Another account's queued work is skipped and **kept**, never deleted, until that person signs back in; the pending badge is scoped the same way so it can't stick. Anything already queued before this change is treated as yours and syncs as before — nothing is stranded. Groundwork for proper multi-user support
- fix: **"Download next N ions" downloaded the wrong ions.** It always paged from the start of the dataset, so on a half-annotated dataset the entire offline copy was ions you had already finished — then going offline and tapping Resume filtered them all out and showed a triumphant "All done!" over an untouched backlog. It now starts at your first unannotated ion (still snapshotting annotated ones too, so Review-all and Review-by-label keep working offline)
- fix: running out of *downloaded* ions no longer claims you finished the dataset — it now says "End of your offline copy" and suggests reconnecting or downloading more
- fix: annotating or starring the same ion twice at once could 500. Both endpoints did a `SELECT` then an `INSERT`-or-`UPDATE`, so two concurrent requests both saw no row, both inserted, and the second hit the unique constraint. Reachable today when the offline sync flush replays a queued annotation while you tap the same ion. `annotate` is now a real `INSERT ... ON CONFLICT DO UPDATE` (what ADR-005 always specified) and `star` is idempotent, so a lost race converges instead of erroring — and stays correct as more devices and users are added
